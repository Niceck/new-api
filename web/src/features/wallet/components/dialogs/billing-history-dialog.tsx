...
interface BillingHistoryDialogProps {
  ...
  onOpenTronOrder?: (tradeNo: string) => Promise<boolean>
}
                ...
                {records.map((record) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {record.payment_method === 'tron' &&
                      (!isAdmin || record.user_id === userID) &&
                      onOpenTronOrder ? (
                        <Button
                          type='button'
                          variant='outline'
                          className='mt-3'
                          onClick={async () => {
                            if (await onOpenTronOrder(record.trade_no)) {
                              onOpenChange(false)
                            }
                          }}
                        >
                          {t('Payment details and claim')}
                        </Button>
                      ) : null}
                      {/* Admin Actions */}
                      {isAdmin &&
